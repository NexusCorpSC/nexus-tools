...
function RecipeEditor({
  ...
  function add() {
    setRows((r) => [...r, { component: "", quantity: "", unit: "" }]);
  }
    ...
    <div className="space-y-2">
      ...
      {rows.map((row, i) => (
        <div key={i} className="flex gap-2 items-center">
          ...
          />
          <Input
            placeholder={tLabels.componentUnit}
            value={row.unit}
            onChange={(e) => update(i, "unit", e.target.value)}
            className="w-24"
          />
...
export function AdvancedBlueprintInputs({
  ...
  return (
    <>
      ...
      {/* Crafting time */}
      <div className="space-y-1.5">
        <Label htmlFor="craftingTime">{tLabels.fieldCraftingTime}</Label>
        <div className="flex items-center gap-2">
          <Input
            ...
            type="number"
            min={0}
            defaultValue={initialCraftingTime ?? ""}
            className="w-40"
            placeholder="0"
          />
          <span className="text-sm text-gray-500">{tLabels.seconds}</span>
        </div>
      </div>
