...
export default function NewJobForm() {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (parsedDuration === null) {
        throw new Error(
          "Invalid duration format. Use numbers for minutes or format like '1d12h30m15s'",
        );
      }
      ...
      toast.error("Error", {
        description:
          error instanceof Error ? error.message : "Failed to add refining job",
      });
    } finally {
  ...
  const handleDurationChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (parsedDuration === null) {
      setDurationError(
        "Invalid format. Use minutes or format like '1d12h30m15s'",
      );
    } else {
      setDurationError(null);
    }
      ...
      <div>
        ...
        {durationInput && !durationError && parseDuration(durationInput) && (
          <p className="mt-1 text-sm text-green-600">
            {t("durationEquivalent", {
              minutes: parseDuration(durationInput)!,
            })}
          </p>
        )}
      </div>
